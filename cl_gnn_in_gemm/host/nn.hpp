...
float relational_model(
	cl_command_queue queue, cl_mem BUF_x_arr, cl_mem BUF_x_out,
	int x_w, int x_h, int RM_w0, int RM_w2, int RM_w4, int RM_w6,
...
float object_model(
	cl_command_queue queue, cl_mem BUF_x_arr, cl_mem BUF_x_out,
	int x_w, int x_h, int OM_w0, int OM_w2, int OM_w4,
...
vector<vector<dtype>> ndr_forward(
	vector<vector<dtype>> obj,
	...
	float* elapsed_time)
{
	...
	*elapsed_time += aggregate_cat(queue1, BUF_obj_arr_t, BUF_effect_receiver_arr_t, BUF_agg_arr,
		obj_t_w, obj_t_h, effect_receiver_h, effect_receiver_w);
